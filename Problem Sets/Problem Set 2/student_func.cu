                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for (int i = 0; i < filterWidth; ++i) {
    for (int j = 0; j < filterWidth; ++j) {
      int pos_x = min(max(0, x - filterWidth / 2 + j), numCols - 1);
      int pos_y = min(max(0, y - filterWidth / 2 + i), numRows - 1);
      new_value += filter[i * filterWidth + j] * inputChannel[pos_y * numCols + pos_x]; 
    }
  }
  outputChannel[offset] = new_value;
}
